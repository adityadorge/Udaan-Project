...
''' This will be executed with fanout '''
def collate_tokens(
    values,
    key,
    pad_idx,
    ...
    if not consnmt:
        ...
        for v in values:
            ...
            max_sub_cons_size = max(max_sub_cons_size, eos_pos - mx[-1])
    ...
    if key == 'source':
        ...
        res_fanout_1 = values[0].new(len(values), src_size+cons_size+2).fill_(0)
        res_fanout_n = values[0].new(len(values), src_size+cons_size+2).fill_(0)  

        def check(p):
            if(((p==5).nonzero()).numel()): # agar isep  hai segment me to if wale me jao 
                x = p >isep_idx
                flag =2
            else: # isep nahi hai to isme jao
                x = p >pad_idx
                flag = 1
        ...
        for i, v in enumerate(values):       
            if consnmt and cons_size > 0:
                ...
                mx = get_sep_posi(v)
                mix = get_isep_posi(v)
                eos_pos = get_eos_posi(v) # 37 
                cons_len = len(mx)
                sep_pos = mx[0]
                # print('v is ',v )
                # print('mx ', mx)
                # print('sep pos', sep_pos )
                ...
                if sep_pos < len(v):
                    ...
                    for pos in range(cons_len):
                        ...
                        if(pos==cons_len-1):
                            # print('mx ', mx)
                            # print('pos', pos )
                            # print('cons_pos last', cons_pos)
                            # print('mix', mix)
                            # print('eos_pos ',eos_pos)
                            # copy_tensor(v[mx[pos]:], res[i][cons_pos:cons_pos + (mix - pos) + 2])
                            # print('len(v[mx[pos]:]' , len(v[mx[pos]:]))
                            # print('eos_pos - mx[pos] ', eos_pos - mx[pos])
                            # copy_tensor(v[mx[pos]:-1], res[i][mx[pos]:eos_pos])
                            copy_tensor(v[mx[pos]:-1], res[i][cons_pos+1: cons_pos + 1 + (eos_pos - mx[pos])])
                            ...
                            # print('flag is ', flag)
                            if(flag == 1):
                                copy_tensor(x, res_fanout_1[i][cons_pos+2:cons_pos +x.shape[0]+2])
                            else:
                                copy_tensor(x, res_fanout_n[i][cons_pos+2:cons_pos+  x.shape[0]+2])
                        else:
                            # print('inside else , pos is ', pos)
                            # print('mx[pos]' , mx[pos])
                            # print('mx[pos+1]' , mx[pos+1])
                            # print('cons_pos ', cons_pos)
                            copy_tensor(v[mx[pos]:mx[pos+1]], res[i][cons_pos + 1:cons_pos + 1 + (mx[pos+1]-mx[pos])])
                            x,flag = check(v[mx[pos]+1:mx[pos+1]])
                            # print('x is ',x)
                            if(flag == 1):
                                copy_tensor(x, res_fanout_1[i][cons_pos+2:cons_pos + 1 +  (mx[pos+1]-mx[pos])])
                            else:
                                copy_tensor(x, res_fanout_n[i][cons_pos+2:cons_pos + 1 + (mx[pos+1]-mx[pos])])
...
''' Uncomment below for running without fanout '''
# def collate_tokens(
#     values,
#     key,
#     pad_idx,
...
#     #print("VALUES shape in data_utils",len(values))

#     '''' Each value is a mini batch ie. value = sample of shapes like [128,624],[80,652] etc'''
...
#                 for i in range(cons_len-1):
#                     max_sub_cons_size = max(max_sub_cons_size,mx[i+1]-mx[i])
#             max_sub_cons_size = max(max_sub_cons_size,v.shape[0]-mx[-1])

#         src_size = max_src_size
#         sub_cons_size = max_sub_cons_size
#         cons_size = (max_cons * sub_cons_size) + 1
...
#         #         if sep_pos < len(v):
#         #             v_src=v[:sep_pos]
#         #             v_src[[v_src[:]>len(src_dict)]] = 3 #Souvik, id of unk token
#         #             # v_cons=v[sep_pos:]                
#         #             copy_tensor(v[:sep_pos], res[i][:len(v_src)])
#         #             # copy_tensor(v[sep_pos:], res[i][src_size:src_size+len(v_cons)])  
#         #             ''' Huh?????????????????????????????????????????????????????????'''             
#         #             for pos in range(cons_len): #SOUVIK earlier it was range(cons_len-1)
#         #                 # v_sub_cons = v[mx[pos]:mx[pos+1]]
#         #                 if(pos==cons_len-1):
#         #                     copy_tensor(v[mx[pos]:], res[i][cons_pos:cons_pos + (mix - pos) + 2])
#         #                 else:
...
#         #                 cons_pos = cons_pos + sub_cons_size
#         #         else:
#         #             v[[v[:]>len(src_dict)]] = 3 #Souvik, id of unk token
#         #             copy_tensor(v, res[i][src_size - len(v):] if left_pad else res[i][:len(v)])
#         #     else:
#         #         v[[v[:]>len(src_dict)]] = 3 #Souvik, id of unk token
#         #         copy_tensor(v, res[i][src_size - len(v):] if left_pad else res[i][:len(v)])

#         # #res = res[res[:,:src_size]<=len(src_dict)]
#         # return res
...
#                         # v_sub_cons = v[mx[pos]:mx[pos+1]]
#                         if(pos==cons_len-1):
#                             copy_tensor(v[mx[pos]:], res[i][cons_pos:cons_pos + (v[mx[pos]:]).shape[0]])
#                             x,flag = check(v[mx[pos]+1:])
#                             if(flag == 1):
#                                 copy_tensor(x, res_fanout_1[i][cons_pos+1:cons_pos+x.shape[0]+1])
#                             else:
...
def load_indexed_dataset(
    path, dictionary=None, dataset_impl=None, combine=False, default="cached", tgt_dict=None
):
